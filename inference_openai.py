...
with open(csv_file_path, 'w', newline='', encoding='utf-8') as csv_file:  
    ...
    for audio_file in audio_files:  
        ...
        # 打印結果  
        print(f"inference time: {infer_time:.4f} | gt: {gt} | ori_pred: {ori_pred} | text: {pred} | wer: {wer_value:.2%} | cer: {cer_value:.2%} | rtf: {rtf_value:.4F}")  
          
        # 寫入結果到 CSV 文件  
        csv_writer.writerow([os.path.basename(audio_file), gt, ori_pred, pred, f"{infer_time:.4f}", f"{wer_value:.2%}", f"{cer_value:.2%}", f"{rtf_value:.4F}"])  
    ...
    # 打印和寫入平均值到 CSV 文件  
    print(f"Average Inference Time: {avg_infer_time:.4f} | Average WER: {avg_wer:.2%} | Average CER: {avg_cer:.2%} | Average RTF: {avg_rtf:.4F}")  
